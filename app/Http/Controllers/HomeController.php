...
class HomeController extends Controller
{
    ...
    public function profileUpdate(Request $request, $id)
    {
        ...
        if ($status) {
            request()->session()->flash('success', 'Sửa hồ sơ thành công');
        } else {
    ...
    public function userOrderDelete($id)
    {
        ...
        if ($order) {
            if ($order->status == "process" || $order->status == 'delivered' || $order->status == 'cancel') {
                return redirect()->back()->with('error', 'Bạn không thể xóa đơn này');
            } else {
                $status = $order->delete();
                if ($status) {
                    request()->session()->flash('success', 'Xóa đơn thành công');
                } else {
                    request()->session()->flash('error', 'Đơn không thể bị xóa');
                }
                return redirect()->route('user.order.index');
            }
        } else {
            request()->session()->flash('error', 'Không tìm thấy đơn');
            return redirect()->back();
        }
    }
